...
class UploadHandler
{
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error,
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            $file->size = $file_size;
            $this->addimage_db($file->name);
            $this->set_file_delete_properties($file);
        }
        return $file;
    }
    
    function addimage_db($imagename)
    {
	    $db = new \PDO("mysql:hostname=127.0.0.1;port=8889;dbname=glmt", "root", "root");
		$sqlst = "insert into images(image_name)values(:image_name)";
		$st = $db->prepare($sqlst);
		$results = $st->execute(array(":image_name"=>$imagename));
    }
    ...
    function deleteimage_db($imagename)
    {
	    $db = new \PDO("mysql:hostname=127.0.0.1;port=8889;dbname=glmt", "root", "root");
		$sqlst = "delete from images where image_name = :image_name";
		$st = $db->prepare($sqlst);
		$results = $st->execute(array(":image_name"=>$imagename));
    }
    ...
    public function delete($print_response = true) {
        ...
        if ($success) {
            foreach($this->options['image_versions'] as $version => $options) {
                if (!empty($version)) {
	                //call the function here for the delete
	                $this->deleteimage_db($file_name);
